    ...
    switch (request.head.method) {
        ...
        .POST => {
            ...
            switch (route) {
                ...
                .complete_stage => |stage_id| return changeStage(
                    allocator,
                    &request,
                    database,
                    stage_id,
...
fn addStage(
    ...
    _ = stages.addCustom(allocator, database, process_id, name) catch |err| {
        if (err == error.InvalidStageName) {
            return respondTimeline(
                ...
                .{
                    .add_stage = .{
                        .name = name,
                        .error_message = "Enter a stage name of 120 characters or fewer.",
                    },
                },
            );
        }
...
fn changeStage(
    ...
    const process_id = switch (change) {
        .complete => stages.complete(allocator, database, stage_id),
...
fn parseRoute(method: std.http.Method, target: []const u8) Route {
    ...
    if (std.mem.eql(u8, resource, "stages") and method == .POST) {
        ...
        if (std.mem.eql(u8, name, "complete")) return .{ .complete_stage = id };
...
fn assignFormValue(
    ...
    } else if (std.mem.eql(u8, key, "company_name")) form.process.company_name = value else if (std.mem.eql(u8, key, "position_name")) form.process.position_name = value else if (std.mem.eql(u8, key, "job_url")) form.process.job_url = value else if (std.mem.eql(u8, key, "source")) form.process.source = value else if (std.mem.eql(u8, key, "location")) {
        form.process.location = value;
        form.appointment.location = value;
    } else if (std.mem.eql(u8, key, "work_arrangement")) form.process.work_arrangement = value else if (std.mem.eql(u8, key, "salary_discussed")) form.process.salary_discussed = true else if (std.mem.eql(u8, key, "salary_min")) assignSalaryValue(&form.process, value, .minimum) else if (std.mem.eql(u8, key, "salary_max")) assignSalaryValue(&form.process, value, .maximum) else if (std.mem.eql(u8, key, "currency")) {
        for (value) |*character| character.* = std.ascii.toUpper(character.*);
        form.process.currency = value;
    } else if (std.mem.eql(u8, key, "period")) form.process.period = value else if (std.mem.eql(u8, key, "salary_type")) form.process.salary_type = value else if (std.mem.eql(u8, key, "salary_notes")) form.process.salary_notes = value;
}
...
test "workflow routes parse explicitly" {
    ...
    try std.testing.expectEqual(
        @as(i64, 7),
        parseRoute(.POST, "/stages/7/complete").complete_stage,
    );
    try std.testing.expectEqual(
        @as(i64, 8),
    ...
    try std.testing.expect(
        parseRoute(.GET, "/stages/7/complete") == .not_found,
    );
}
